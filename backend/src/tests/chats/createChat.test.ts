...
describe('POST /chats', () => {
  ...
  it('should return 201 and create chat with userId of authenticate user', async () => {
    const { body } = await supertest(server)
      .post('/chats')
      .set('Authorization', user.bearerToken)
      .expect(201)
    
    const savedChat = await chatModel.findById(body._id)
    expect(savedChat).toBeTruthy()
    expect(savedChat?.patient.toString()).toBe(user.user._id?.toString())
    expect(body.patient).toBe(user.user._id?.toString())
  })
  ...
  it('should return 403 if user is authenticated as admin', async () => {
    await supertest(server)
      .post('/chats')
